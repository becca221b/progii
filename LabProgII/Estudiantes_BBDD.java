...
public class Estudiantes {
    ...
    private static void consultar_mayor_de(Connection conexion, Scanner scanner)throws SQLException{
        System.out.print("Mayores de qué edad desean saber");
        int edad_buscada = scanner.nextInt();
        scanner.nextLine(); // Consumir la nueva línea después de leer un entero.

        String consulta = "SELECT DATEDIFF(YEAR, fecha_nacimiento, WHERE GETDATE()) > ? FROM estudiantes;";

        PreparedStatement preparedStatement = conexion.prepareStatement(consulta);
        preparedStatement.setInt(1, edad_buscada);
